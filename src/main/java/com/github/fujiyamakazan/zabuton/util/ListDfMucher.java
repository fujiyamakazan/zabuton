...
public abstract class ListDfMucher<T> {
     ...
     */
    public void execute() {
        for (Iterator<T> iteA = this.restLeft.iterator(); iteA.hasNext();) {
            T a = iteA.next();
            for (Iterator<T> iteB = this.restRight.iterator(); iteB.hasNext();) {
                T b = iteB.next();
                if (eq(a, b)) {
                    /* ペアとして登録する */
                    this.pairs.add(new KeyValueObj<T, T>(a, b));
